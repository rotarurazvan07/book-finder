...
def getRating(book, similarity_engine):
    rating = None
    ...
    # 1. ISBN attempt
    if isbn:
        request_result = WebScraper.fetch(GOODREADS_SEARCH % isbn, stealthy_headers=True)
        try:
            if request_result:
            ...
            if request_result:
                if NOT_FOUND_INDICATOR not in request_result.lower():
                    ...
                    for book_elem in book_elements:
                        ...
                        match, _ = similarity_engine.is_similar(title, goodreads_title)
                        if match and author and goodreads_author:
                            match, _ = similarity_engine.is_similar(author, goodreads_author)
