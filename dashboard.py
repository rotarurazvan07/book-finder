...
class BookDashboard:
    ...
    def _setup_layout(self) -> None:
        self.app.layout = dbc.Container(
                                                ...
                                                dbc.Col(
                                                    [
                                                        html.H2(
                                                            [
                                                                html.I(
                                                                    className="fas fa-book-reader me-3"
                                                                ),
                                                                "Live Book Monitor",
                                                            ],
                                                        ...
                                                        html.P(
                                                            "Smart Antiquarian Scraper & Analyzer",
                                                ...
                                                dbc.Col(
                                                    [
                                                        dbc.Button(
                                                            [
                                                                html.I(
                                                                    className="fas fa-redo-alt me-2"
                                                                ),
                                                                "Refresh Data",
                                                            ],
                                                            id="refresh-btn",
                                            ...
                                            [
                                                ...
                                                dcc.RangeSlider(
                                                    ...
                                                    value=[
                                                        2,
                                                        100,
                                                    ],  # This is the secret for 2 heads
                                                    tooltip={
                                                        "always_visible": True,
                                                        "placement": "bottom",
                                                    },
            ...
            fluid=True,
            className="bg-light min-vh-100 px-3",
        )
    ...
    def _setup_callbacks(self) -> None:
        ...
        def update_view(
            n_clicks, search, selected_cats, selected_stores, price_range, min_rating
        ):
            ...
            if search:
                f_df = f_df[
                    f_df["title"].str.contains(search, case=False, na=False)
            ...
            if selected_cats:
                f_df = f_df[
                    f_df["category"].apply(
                        lambda x: (
                            any(cat in x for cat in selected_cats)
            ...
            if isinstance(price_range, list) and len(price_range) == 2:
                f_df = f_df[
                    (f_df["price"] >= price_range[0])
                    & (f_df["price"] <= price_range[1])
                ]

            f_df["price"] = f_df["price"].round(2)
            f_df["rating"] = f_df["rating"].apply(
                lambda x: round(x, 2) if pd.notnull(x) else x
            )
            f_df["cat_display"] = f_df["category"].apply(
                lambda x: ", ".join(x) if isinstance(x, list) else ""
            )
            f_df["store_md"] = f_df["url"].apply(lambda x: f"[Buy]({x})")
            f_df["gr_md"] = f_df["goodreads_url"].apply(
                lambda x: f"[GR]({x})" if x and str(x) != "None" else "N/A"
            )

            cat_opts = [
                {"label": str(c), "value": c}
                for c in sorted(df["category"].explode().dropna().unique())
            ]
            store_opts = [
                {"label": str(s), "value": s}
                for s in sorted(df["store"].dropna().unique())
            ]
    ...
    def run(self, debug: bool = True, port=8050) -> None:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "db_path", help="Path to the SQLite database file (e.g., books.db)"
    )
